...
class CameraLoader1(val activity: Activity) : CameraLoader() {
    ...
    override fun takePicture(onPictureTaken: (data: ByteArray) -> Unit) {
        ...
        when (cameraInstance!!.parameters.focusMode) {
            Camera.Parameters.FOCUS_MODE_FIXED -> cameraInstance!!.takePicture(
                null, null
            ) { data, _ ->
                onPictureTaken(data)
            }
            Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE -> cameraInstance!!.takePicture(
                null,
                null
            ) { data, _ ->
                onPictureTaken(data)
